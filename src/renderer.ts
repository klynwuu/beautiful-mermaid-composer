import type { PositionedGraph, PositionedNode, PositionedEdge, PositionedGroup, Point, ConnectorStyle } from './types.ts'
...
import { pointsToPolylinePath, pointsToCurvedPath, round } from './edge-path.ts'
...
export function renderSvg(
  ...
  for (const edge of graph.edges) {
    parts.push(renderEdge(edge, edgeStyle))
  }
...
function renderEdge(edge: PositionedEdge, edgeStyle: ConnectorStyle = 'sharp'): string {
  if (edge.points.length < 2) return ''

  const curved = edgeStyle === 'curved' && edge.points.length > 2
  ...
  if (curved) {
    const d = pointsToCurvedPath(edge.points)
    return (
      `<path ${dataAttrs.join(' ')} d="${d}" fill="none" stroke="${strokeColor}" ` +
      `stroke-width="${strokeWidth}"${dashArray}${markers} />`
    )
  }

  return (
    `<polyline ${dataAttrs.join(' ')} points="${pointsToPolylinePath(edge.points)}" fill="none" stroke="${strokeColor}" ` +
    `stroke-width="${strokeWidth}"${dashArray}${markers} />`
  )
}
